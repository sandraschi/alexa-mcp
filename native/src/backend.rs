...
const BACKEND_NAME: &str = "alexa-mcp-backend.exe";
const BACKEND_PORT: u16 = 10801;
const BACKEND_TAG: &str = "alexa-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "ALEXA_MCP_PORT";
const ENV_HOST: &str = "ALEXA_MCP_HOST";
const ENV_TAURI: &str = "ALEXA_MCP_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'alexa-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'alexa-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM alexa-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM alexa-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name alexa-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM alexa-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    let addr: std::net::SocketAddr = format!("127.0.0.1:{BACKEND_PORT}")
        .parse().map_err(|e| format!("Invalid addr: {e}"))?;
