...
export function FleetCard({ member, currentAppId }: FleetCardProps) {
    ...
    const checkStatus = async () => {
        try {
            ...
            const resp = await fetch(`http://localhost:${member.port}/api/status`, { signal: controller.signal });
            clearTimeout(id);
    ...
    const launchApp = async () => {
        ...
        setError(null);
        try {
            const resp = await fetch(api.fleetLaunch, {
                method: 'POST',
            ...
            const poll = setInterval(async () => {
                attempts++;
                try {
                    const check = await fetch(`http://localhost:${member.port}/api/status`);
                    if (check.ok) {
                        clearInterval(poll);
