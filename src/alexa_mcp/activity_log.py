"""In-memory event log for web dashboard — tail, filter, export, rotation (fleet v1.0)."""
...
_DEFAULT_MAX = 2000
_max_entries = max(100, min(int(os.environ.get("ALEXA_LOG_MAX_ENTRIES", str(_DEFAULT_MAX))), 50_000))
_lock = Lock()
...
def export_logs(
    *,
    ...
    if format == "csv":
        ...
        return buffer.getvalue(), "text/csv", f"alexa-mcp-logs-{stamp}.csv"

    body = json.dumps({"exported_at": datetime.now(UTC).isoformat(), "entries": entries}, indent=2)
    return body, "application/json", f"alexa-mcp-logs-{stamp}.json"
